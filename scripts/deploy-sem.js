...
async function deploy_semaphore() {
  const [deployer, proxyAdmin, relayer] = await ethers.getSigners();
  let nonce = await deployer.getTransactionCount();

  const Pool = await ethers.getContractFactory("Pool");
  const SemaphoreOperatorManager = await ethers.getContractFactory("SemaphoreOperatorManager");
  ...
  const semaphoreOperatorManager = await SemaphoreOperatorManager.deploy(100, 20, {nonce: nonce++});
  await semaphoreOperatorManager.deployed();
  ...
  const pool = await Pool.deploy(poolId, tokenAddress, voucherTokenAddress, "1000000000", "1000000000", "1000000000", 
      transferVerifier.address, treeVerifier.address, semaphoreOperatorManager.address, initialRoot, {nonce: nonce++});
  ...
  return {
    //pool:poolProxified,
    //Pool
    operatorManager: semaphoreOperatorManager,
    SemaphoreOperatorManager,
  };
}

module.exports = deploy_semaphore;
